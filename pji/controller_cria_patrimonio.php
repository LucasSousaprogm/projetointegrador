...
try {
    ...
    if ($e->getCode() === '23000' && strpos($e->getMessage(), 'Duplicate entry') !== false) {
        $erro = "Erro: O código do patrimônio **" . htmlspecialchars($codigo) . "** já existe.";
    } else {
        error_log("Erro ao cadastrar patrimônio: " . $e->getMessage());
        $erro = "Erro interno ao cadastrar patrimônio. Tente novamente.";
    }
    header('Location: ../views/view_cria_patrimonio.php?erro=' . urlencode($erro));
    exit;
}